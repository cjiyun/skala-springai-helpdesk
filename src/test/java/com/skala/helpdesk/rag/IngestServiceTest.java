...
class IngestServiceTest {
  ...
  void 재인제스트하면_해당_source만_최신_내용으로_교체한다() {
    ...
    IngestService service = new IngestService(vectorStore);

    service.ingest(new ClassPathResource("helpdesk-docs/return-policy.md"), "policy", "1");
    IngestResponse membership = service.ingest(
        new ClassPathResource("helpdesk-docs/membership.md"), "membership", "1");
    IngestResponse reindexed = service.ingest(
        new ClassPathResource("helpdesk-docs/shipping-policy.md"), "policy", "2");
